...
**出现原因**  
React 的响应式原理导致组件更新的时候会生成更大的虚拟 DOM 树，当应用组件数量庞大，Stack 结构一旦开始比较就**无法中断**的问题，会导致长期占用主线程，形成阻塞，造成卡顿现象。  
React Fiber 并不会减少比较时间，只是会保存工作进度，可断点重启。
> 原来的**树结构**，只有指向子节点。一旦中断，保存的正在处理的节点无法找到父节点。

Fiber 是一个执行单元。是纤程。  
纤程是协程的一种实现方式。协程比线程更小的调度单位。开启、暂停可以被程序员控制。  
> 底层使用类似 requestIdleCallback ，自己实现的 api
