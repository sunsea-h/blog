...
| ------------ | -------------------------------------------------- | ----------------------------------------------------------------- |
| `break-word` | 以「完整单词」为单位计算最小宽度（超长单词算整体） | 元素会优先尝试「撑到完整单词宽度」，再换行，flex 子元素可能更宽； |
| `anywhere`   | 以「单个字符」为单位计算最小宽度                   | 元素会尽可能收缩到最小宽度（单个字符宽度），flex 子元素更紧凑；   |
